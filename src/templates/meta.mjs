...
${hubHead({
    ...
    <table class="vtable">
      ...
      <tbody>
        ...
        <tr><th scope="row"><a href="${base}today.json">today.json</a></th><td>One entry, chosen deterministically from the date, for anything that wants an entry of the day without scraping a page.</td><td>JSON</td></tr>
        <tr><th scope="row"><a href="${base}print/">print/</a></th><td>The whole index as one document: the claim, the verdict and the reason for it, grouped by field, for paper or a PDF. Sources stay on the entry pages.</td><td>HTML</td></tr>
        <tr><th scope="row"><a href="${base}embed/">embed/&lt;slug&gt;/</a></th><td>Any entry as a self-contained card for another site, in one line of HTML. No script, no stylesheet fetch, follows light or dark.</td><td>HTML</td></tr>
    ...
    jsonld: [
      ...hubJsonLd({ name: 'Download the dataset', description, path: 'data/', origin, base, crumbs: [] }),
      {
        ...
        distribution: [
          ...
          { '@type': 'DataDownload', encodingFormat: 'application/json', contentUrl: `${origin}${base}today.json` },
